...
esphome::Color hsv_to_rgb(uint8_t hue, uint8_t sat, uint8_t val) {
    ...
    const uint8_t third = esphome::esp_scale8(offset8, 85);
    const uint8_t two_thirds = esphome::esp_scale8(offset8, 170);
    esphome::Color rgb(255, 255, 255, 0);
    switch (hue >> 5) {
        case 0b000:
            rgb.r = 255 - third;
            rgb.g = third;
    ...
    rgb += esphome::esp_scale8(desat, desat);
    // (r,g,b) = (r,g,b) * val
    rgb *= val;
    return rgb;
}
